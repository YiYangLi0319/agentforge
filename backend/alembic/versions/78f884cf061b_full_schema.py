...
def upgrade() -> None:
    # 托管 PostgreSQL 可能无 CREATE EXTENSION 权限。失败时保持事务可用并以 JSON 建向量列；
    # 后续应用启动会读取实际列类型，而不会因扩展状态变化在 JSON/vector 间翻转。
    bind = op.get_bind()
    agentforge.db.types.PGVECTOR["enabled"] = False
    if bind.dialect.name == "postgresql":
        op.execute(
            ...
            BEGIN
                ...
                EXCEPTION WHEN OTHERS THEN
                    RAISE NOTICE 'pgvector unavailable, falling back to JSON: %', SQLERRM;
                END;
            END
            $do$;
            """
        ...
        agentforge.db.types.PGVECTOR["enabled"] = bool(has_vector)
