...
class HybridRetriever:
    def __init__(
        ...
        json_scan_limit: int = 5000,
    ):
        self.sessions = sessions
        self.embeddings = embeddings
        self.reranker = reranker
        self.json_scan_limit = json_scan_limit
    ...
    async def _vector_search(
        ...
        *,
        ...
        # SQLite / 其他方言：进程内余弦。小库全量扫描；超过上限时用 BM25 候选预筛，防止 OOM。
        scan_stmt = select(Chunk.id, Chunk.embedding).where(
        ...
        if total > self.json_scan_limit:
            candidate_ids: list[str] = []
            if query_terms:
                index = await self._get_bm25(session, kb_ids)
                candidate_ids = [cid for cid, _ in index.search(query_terms, top_k=self.json_scan_limit)]
            if candidate_ids:
                scan_stmt = scan_stmt.where(Chunk.id.in_(candidate_ids))
                logger.warning(
                    "JSON 向量库规模 %s 超过扫描上限 %s，改用 BM25 候选(%s)预筛；生产建议启用原生 pgvector",
                    total, self.json_scan_limit, len(candidate_ids),
                )
            else:
                scan_stmt = scan_stmt.limit(self.json_scan_limit)
                logger.warning(
                    "JSON 向量库规模 %s 超过扫描上限 %s 且无关键词候选，截断扫描",
                    total, self.json_scan_limit,
                )
        ...
        async with self.sessions() as session:
            ...
            if mode in ("hybrid", "vector"):
                ...
                vector_hits = await self._vector_search(
                    session, kb_ids, query_emb, candidate_n, query_terms=terms
                )
            if mode in ("hybrid", "keyword"):
                index = await self._get_bm25(session, kb_ids)
                bm25_hits = index.search(terms, top_k=candidate_n)
