...
from agentforge.rag.citations import extract_cited_ids
...
async def run_deep_research(
    ...
    *,
    ...
    try:
        ...
        yield SourcesUpdated(sources=sources[:max_sources])
        if not findings:
        ...
        async def review_report(md: str) -> Review:
        ...
        def score_of(rv: Review) -> int:
            return rv.completeness + rv.citation_quality + rv.logic

        def emit_review(rv: Review) -> ReportReview:
            return ReportReview(
                passed=rv.passed,
                scores={
        ...
        review = await review_report(report_md)
        yield emit_review(review)
        # 保底：记录历次评分最高的版本，即使多轮仍不达标也交出最好的一版
        best_md, best_review, best_score = report_md, review, score_of(review)
        ...
        while not review.passed and revision < max_revisions:
            ...
            async for piece in write_report(
                ...
                f"重点补强被扣分的维度（尤其是引用规范：事实句务必标注真实存在的 [n]）：\n{review.feedback}"
            ):
                if isinstance(piece, tuple):
                    revised = piece[1]
                else:
        ...
        if not review.passed:
            report_md, review = best_md, best_review

        # ---------- 收尾：附加参考来源，汇总用量 ----------
        used_ids = extract_cited_ids(report_md)
        final_sources = [s for s in sources if s["id"] in used_ids] or sources[:max_sources]
        refs = "\n".join(
